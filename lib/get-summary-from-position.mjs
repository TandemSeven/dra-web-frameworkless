...
export default function getSummaryFromPosition(position) {
	const latlng = position.latitude + ',' + position.longitude;

	const promise = summaryByLatLng[latlng] = summaryByLatLng[latlng] || Promise.all([
		fetch('https://api.weather.gov/points/' + latlng)
		.then(response => response.json()),
		fetch('https://api.teleport.org/api/locations/' + latlng + '/?embed=location:nearest-urban-areas/location:nearest-urban-area/ua:images')
		.then(response => response.json())
	]).then(json => {
		...
		return getForecastFromURL(forecastURL).then(
			forecast => ({
				place: json[0].properties.relativeLocation.properties.city,
				state: json[0].properties.relativeLocation.properties.state,
				imageURL: imageURL,
				forecast: forecast
			})
		);
